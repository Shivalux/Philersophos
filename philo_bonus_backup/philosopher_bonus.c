...
/*   Updated: 2022/09/06 12:19:32 by sharnvon         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_philo_routine(t_table *table)
{
	...
	while (table->philo_status != DEAD)
	{
		...
		ft_isleepnow(table->eat);
		sem_wait(table->philo[table->count].sem_lifetime);
		table->philo[table->count].life = table->life_time;
		sem_post(table->philo[table->count].sem_lifetime);
		table->philo[table->count].meal++;
		sem_post(table->sem_fork);
		sem_post(table->sem_fork);
		if (table->meal != -1 && table->philo[table->count].meal >= table->meal)
			break ;
